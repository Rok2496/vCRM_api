...
export class ApplicationRolesController {
  ...
  @HasPermissions([PermissionTitle.APP_ROLE_CREATE])
  ...
  @UsePipes(new ValidationPipe(true))
  @UsePipes(new TrimPipe())
  @HasPermissions([PermissionTitle.APP_ROLE_UPDATE])
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @Patch(':id')
  async update(
    @Param('id') id: number,
  ...
  @ApiBearerAuth()
  @UsePipes(new ValidationPipe(true))
  @HasPermissions([PermissionTitle.APP_ROLE_DELETE])
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @Delete(':id')
  ...
  @Patch('manage-permissions/:id')
  async managePermissionsByRoleId(
    @Param('id') id: number,
    @User() user: IUser,
    @Body() data: ManagePermissionsDto,
  ) {
    try {
      return this.service.managePermissionsByRoleId(id, user, data);
    } catch (err) {
      throw new HttpException(err, err.status || HttpStatus.BAD_REQUEST, {
        cause: new Error(err),
      });
  ...
  async permissionsCheckerByRoleId(
    ...
    try {
      return this.service.permissionsCheckerByRoleId(id, user, query);
    } catch (err) {
      throw new HttpException(err, err.status || HttpStatus.BAD_REQUEST, {
        cause: new Error(err),
      });
