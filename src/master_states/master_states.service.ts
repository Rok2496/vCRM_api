...
import { IMaster_State as IType, IUser } from 'src/common/interfaces';
import { IQuery } from 'src/common/types';
import { Master_States as Entity } from 'src/models';
import { DeleteResult, Repository } from 'typeorm';
import {
  CreateMasterStateDto as CreateDto,
  UpdateMasterStateDto as UpdateDto,
} from './dto';

@Injectable()
export class MasterStatesService {
  /**
   * Constructor
  ...
  async create(user: IUser, data: CreateDto): Promise<IType> {
    try {
      const { master_country, ...rest } = data;
      const payload = {
        ...rest,
