...
import { IMaster_County as IType, IUser } from 'src/common/interfaces';
import { IQuery } from 'src/common/types';
import { Master_Counties as Entity } from 'src/models';
import { DeleteResult, Repository } from 'typeorm';
import {
  CreateMasterCountyDto as CreateDto,
  UpdateMasterCountyDto as UpdateDto,
} from './dto';

@Injectable()
export class MasterCountiesService {
  /**
   * Constructor
  ...
  async create(user: IUser, data: CreateDto): Promise<IType> {
    try {
      const { master_country, master_state, ...rest } = data;
      const payload = {
        ...rest,
