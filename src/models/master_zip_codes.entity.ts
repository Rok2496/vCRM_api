...
@Entity({ name: ENTITY_NAME.MASTER_ZIP_CODE })
export class Master_Zip_Codes extends BaseEntity {
  @Column({ type: 'varchar', length: 128 })
  name: string;

  @RelationId((x: Master_Zip_Codes) => x.master_country)
  master_country_id: number;

  @RelationId((x: Master_Zip_Codes) => x.master_state)
  master_state_id: number;

  @RelationId((x: Master_Zip_Codes) => x.master_county)
  master_county_id: number;

  @ManyToOne(() => Master_Countries, (country) => country.master_zip_codes, {
    nullable: true,
  })
  @JoinColumn({ name: 'master_country_id' })
  master_country: Master_Countries;

  @ManyToOne(() => Master_States, (state) => state.master_zip_codes, {
    nullable: true,
  })
  @JoinColumn({ name: 'master_state_id' })
  master_state: Master_States;

  @ManyToOne(() => Master_Counties, (county) => county.master_zip_codes, {
    nullable: true,
  })
