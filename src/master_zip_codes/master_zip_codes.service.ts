...
import { IMaster_Zip_Code as IType, IUser } from 'src/common/interfaces';
import { IQuery } from 'src/common/types';
import { Master_Zip_Codes as Entity } from 'src/models';
import { DeleteResult, Repository } from 'typeorm';
import {
  CreateMasterZipCodeDto as CreateDto,
  UpdateMasterZipCodeDto as UpdateDto,
} from './dto';

@Injectable()
export class MasterZipCodesService {
  /**
   * Constructor
  ...
  async create(user: IUser, data: CreateDto): Promise<IType> {
    try {
      const { master_country, master_state, master_county, ...rest } = data;
      const payload = {
        ...rest,
        master_country: { id: master_country },
        master_state: { id: master_state },
        master_county: { id: master_county },
      };
  ...
  async update(id: number, user: IUser, data: UpdateDto): Promise<IType> {
    try {
      ...
      const masterCountry = data.hasOwnProperty('master_country')
        ? { id: data.master_country }
        : record.master_country;

      const masterState = data.hasOwnProperty('master_state')
        ? { id: data.master_state }
        : record.master_state;

      const masterCounty = data.hasOwnProperty('master_county')
        ? { id: data.master_county }
        : record.master_county;
